...
function getCategories(lang: Language) {
  return lang === 'en' ? EN_CATEGORIES : NO_CATEGORIES
}

function pickCategoryAndWord(last?: { category?: string; word?: string }, lang: Language = 'no') {
  // Maintain a short history per category to avoid repeats over several rounds.
  ...
  // Prefer a different category than last; if same, ensure different word
  let catCandidates = getCategories(lang)
  if (last?.category) {
    catCandidates = getCategories(lang).filter((c: any) => c.name !== last.category)
    if (catCandidates.length === 0) catCandidates = getCategories(lang)
  }
  ...
  let wordCandidates = cat.words.filter(w => !EXCLUDE_SET.has(w))

  // If filtering removes all, allow all but avoid the immediate last word if same category
  if (wordCandidates.length === 0) {
    wordCandidates = cat.words
    if (last?.category === cat.name && last?.word) {
      const alt = cat.words.filter(w => w !== last.word)
  ...
  // Update history with bounded size per category (keep last 5)
...
export function App() {
  ...
  function initGame(playerCount: number, imposterCount: number, timerEnabled: boolean, timerSeconds: number) {
    ...
    try {
      const raw = localStorage.getItem('imposterwho:last')
      if (raw) last = JSON.parse(raw)
    } catch {}
    const { category, word } = pickCategoryAndWord(last, language)
    const indices: number[] = []
    while (indices.length < imposterCount) {
      const r = randomInt(playerCount)
      if (!indices.includes(r)) indices.push(r)
    }
    const gameState: GameState = {
      playerCount,
      imposterIndices: indices.sort((a,b)=>a-b),
      category,
      secretWord: word,
  ...
  return (
    <main style={{minHeight:'100vh',fontFamily:'system-ui, sans-serif',background:'#111',color:'#fff',display:'grid',placeItems:'center',padding:'24px'}}>
      <div style={{maxWidth:600,width:'100%'}}>
        {phase === 'setup' && (
          <Setup onStart={initGame} language={language} onLanguageChange={(l)=>{ setLanguage(l); try { localStorage.setItem('imposterwho:lang', l) } catch {} }} />
        )}
        ...
        {phase === 'discussion' && state && (
          <Discussion
            category={state.category}
            onStartTimer={startCountdown}
            timerEnabled={state.timerEnabled}
            seconds={state.timerSeconds}
            countdown={countdown}
            onEnd={() => setPhase('reveal-result')}
            language={language}
          />
        )}
        ...
        {phase === 'reveal-result' && state && (
          <Result
            imposterIndices={state.imposterIndices}
  ...
  return (
    <Card title={t(language,'setupTitle')}>
      <div style={{display:'grid',gap:16}}>
        ...
        {!playersValid && (
          <div style={{color:'#fca5a5'}}>{t(language,'playersInvalid')}</div>
        )}
        ...
        {!impostersValid && (
          <div style={{color:'#fca5a5'}}>{t(language,'impostersInvalid').replace('{max}', String(maxImposters))}</div>
        )}
    ...
    <Card title={t(language,'revealTitle').replace('{cur}', String(playerIndex+1)).replace('{total}', String(totalPlayers))}>
      <div style={{display:'grid',gap:24,textAlign:'center'}}>
        <div style={{opacity:visible?1:0,transition:'opacity 300ms'}}>
          ...
          {role === 'Imposter' && (
            <>
              <div style={{fontSize:20,opacity:0.85}}>{t(language,'youAre')}</div>
              <div style={{fontSize:36,fontWeight:700,marginBottom:12}}>{t(language,'imposter')}</div>
              <div style={{opacity:0.85,marginTop:8}}>{t(language,'youDontKnowWord')}</div>
            </>
          )}
          {role !== 'Imposter' && (
            <>
              <div style={{fontSize:20,opacity:0.85}}>{t(language,'wordIs')}</div>
              <div style={{fontSize:40,fontWeight:700}}>{word}</div>
            </>
          )}
        </div>
